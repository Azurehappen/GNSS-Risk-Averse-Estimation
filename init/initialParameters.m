function [p, eph, obs] = initialParameters(p, files, eph, data_p)
...
fields = fieldnames(data_p); % Get all field names from data_p
for i = 1:length(fields)
    field = fields{i}; % Get each field name
    p.(field) = data_p.(field); % Copy the field and its value to B
end
